...
export class PostgresEngine implements BrainEngine {
  ...
  private async _upsertChunksOnce(slug: string, chunks: ChunkInput[], opts?: { sourceId?: string }): Promise<void> {
    ...
    for (const chunk of chunks) {
      ...
      // Allocate the code-embedding index here (after embedding_image, before
      // the regular columns) so the param push order below stays aligned.
      ...
      // (embedding, embedding_image, embedding_code, then the regular columns).
