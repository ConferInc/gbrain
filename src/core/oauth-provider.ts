...
export class GBrainOAuthProvider implements OAuthServerProvider {
  ...
  // [RT5-FREEZE P1 4.2 note] Conflict was at this signature line: HEAD (upstream) declares
  // `Promise<SdkAuthInfo>`; our overlay's pre-rebase base declared `Promise<AuthInfo>` (a bare
  // name that predates this file's current `AuthInfo as SdkAuthInfo` / `AuthInfo as
  // CoreAuthInfo` import aliasing — see imports near the top of this file). Took upstream's
  // signature verbatim: it's the only one that type-checks against the current imports; our
  // getFederatedSourceIds() addition above is unaffected (pure insertion, no signature
  // dependency).
